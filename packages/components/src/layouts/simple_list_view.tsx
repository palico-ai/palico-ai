...
import { Box, Card, Divider, Grid, List } from '@mui/material'
...
const NoItemText: React.FC<{ text: string }> = ({ text }) => {
  return (
    <Box sx={{
      ...
      <Typography variant="h5" textAlign={'center'} whiteSpace={'pre'} gutterBottom>
        {text}
      </Typography>
    </Box>
  )
...
export function SimpleListView<Item> (props: SimpleListViewProps<Item>): ReactNode {
  const {
    card = false,
    items,
    gridView = false,
